...
live_loop :repeating_melody1 do
  ...
  with_fx :reverb, kill_delay: 0.2, room: 0.3 do
    ...
    (m*2).times do
      sleep 0.25
      l = rrand(0.1,0.25) / 2
      play notes.choose-12, attack: l, release: l, cutoff: rrand_i(80, 100), amp: factor?(look, 4) ? 1 : 0.5 if (spread 5, 4).tick(:aticker)
    end
  end
end
...
live_loop :repeating_melody do
  ...
  with_fx :reverb, kill_delay: 0.2, room: 0.3 do
    ...
    (n*2).times do
      sleep 0.25
      l = rrand(0.05, 0.2)
      play notes.choose, attack: l, release: 0.25-l, cutoff: rrand_i(80, 100), amp: factor?(look, 4) ? 1 : 0.5 if (spread 15, 16).tick(:aticker)
    end
  end
end
