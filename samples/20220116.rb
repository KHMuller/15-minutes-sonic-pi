...
# Getting closer ... :)
# Next step ... adjust scale flute/organ to selected base of cello
...
live_loop :base do
  ...
  bs = (scale :c3, :minor, num_octaves: 2)
  drift = (ring 0, 0, 0, 0, 3, 3, 0, 0, 4, 3, 0, 0).tick(:riff)
  
  4.times do
    shift = (ring 0, 2, 4, 7).tick(:basenotes)
    cello(bs[shift+drift], l/2, 1.2, rrand(0,0))
    sleep l
  end
end
...
live_loop :master do
  ...
  flute(n_f+7, l, 0.5+a.choose, -0.5) if (spread 5, 8).tick(:organobeat1)
  organ(n_f, l, 0.5+a.choose, 0.5) if (spread 8, 8).tick(:organobeat1)
  sleep l
